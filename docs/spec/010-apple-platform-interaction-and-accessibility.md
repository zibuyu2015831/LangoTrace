...
- 状态反馈 sheet 的 running / testing 状态应使用 `ProgressView` 和可读状态文案；不可点击的禁用主按钮不应作为进度反馈。完成后才展示重试、查看详情或继续等真实可用操作，并避免把一次性 overall status 文案当作 sheet 顶部标题。成功态不应默认提供 prominent 重试按钮；失败、部分可用、取消或不支持状态才应突出恢复操作。
...
- 练习页的可展开学习辅助信息不得遮挡或挤压主操作。中文释义和讲解默认收起，分别通过 `查看释义` 和 `查看讲解` 显示；两个入口优先同行显示，但 Dynamic Type 或窄屏下必须能换行回退，且每个按钮仍满足 44pt 触控目标。隐藏正文时不得预留大块空白。释义展开后不重复显示冗余标题；讲解展开后多段内容之间应有可感知间距。释义和讲解展开后必须可完整查看，不能只依赖省略号表达不可见内容；展开 / 收起按钮必须提供可访问名称、当前展开状态 value 和操作 hint。Reduce Motion 开启时，展开 / 收起不能依赖动画表达状态变化。
