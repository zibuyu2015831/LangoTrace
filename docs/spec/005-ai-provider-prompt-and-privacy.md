...
Provider 配置页已经从真实级 mock 表单进入本地配置保存和配置合成测试阶段：非敏感 Provider profile、endpoint、credential metadata 和 validation event 进入 SQLite / GRDB，API Key 写入 Keychain；测试请求可以通过 Provider 层对文本 endpoint 发送固定文本、JSON 和可选内置图片合成探测。真实学习内容请求、Prompt Preset 执行、请求预览和请求日志仍未接入。后续实现必须遵守以下边界：
