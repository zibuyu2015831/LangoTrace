...
- OpenAI Responses 和 OpenAI-compatible Chat 是第一阶段真实文本、内置图片合成测试和 OpenAI-like embeddings 配置测试范围；**Anthropic Messages 自 LM03-S4b（2026-06-27）起、Gemini generateContent 自 2026-07-22 起支持文本 + 尽力而为结构化 JSON 探针**（Anthropic：`x-api-key`+`anthropic-version` 鉴权、`content[].text` 解析；Gemini：`x-goog-api-key` 鉴权、model 在路径 `models/{model}:generateContent`、`candidates[].content.parts[].text` 解析）；两者的图片 / TTS / embedding 探针仍返回明确暂不支持，不得误映射为认证失败或网络失败。Provider preset 不能作为模型级图片输入能力或向量能力的最终事实源；OpenRouter 和 Custom OpenAI-compatible 等兼容层应表达为 model-dependent，允许用户显式开启并由真实 probe 验证。`supportsImageInput` 只作为 endpoint 运行期防线和保存快照，不表示模型已被验证支持图片理解；embedding 可用性也以 endpoint scoped probe result 为准。
...
- 是否引入本地内容脱敏或敏感词提示。
- 多轮对话与文本流式的传输能力已落地（见 §3、§8）。**语伴会话已接文本流式 UX**（LM03-S3a：`CompanionConversationEngine.reply(onPartial:)` 逐 delta 累积回调 → `CompanionChatActions.send` seam +`onPartial` 第三参 → store `AsyncStream` 单 MainActor 顺序消费 → in-flight 气泡；流式只改显示，外发请求体 / 内容类目 / 隐私闸与 S1/S2b 完全一致，无新 capability / 无新外发类目）。**语伴对话记忆 / 滚动摘要已落地**（LM03-S3b-1：超窗时把老化轮压缩为摘要注入、`.companionSummarization` capability preview-only 诚实披露、删/清空摘要同事务失效重建；只压缩本会话已外发内容、无新外发类目、无新 consent 门）。**语伴 Style 受控片段注入已落地**（LM03-S4a：surface Style 印记经 Ability band 的 i+1 下投影注入——正式度镜像、复杂度按 band 只读封顶；新 `.curatedLearnerStyle` included descriptor 复用 `.companionConversation` capability，复用 S2b-1 两层 consent 门、**Style 块零原始用户内容故无需 PII scrub**、无新 migration/capability/外发类目语义升级）。**LM03 语伴系列至此全部落地**；剩对话级日志写入接线属可演进。
- **Anthropic Messages 多轮 + 流式适配已落地（LM03-S4b，2026-06-27）**：`AnthropicMessagesTextAdapter`（顶层 `system` / 必填 `max_tokens` / `content[].text` 解析 / `content_block_delta` 流式 / 无 `[DONE]` 由字节 EOF 终止）；鉴权升为可动态派发的协议要求 `providerRequestHeaders`（Anthropic `x-api-key`+`anthropic-version`、mimo `api-key`、OpenAI 默认 Bearer，连带修复 mimo 旧 override 潜伏鉴权 bug）。语伴 send 路径 kind-agnostic、零 App 改动即可跑 Anthropic。**无新外发类目、无新 `AIRequestCapability`**：Anthropic 只是又一个用户自配 Provider 端点，决策 #10 与语伴隐私闸不变；密钥走既有 Keychain，不入日志 / SQLite / 同步。Anthropic 上的严格 schema 结构化输出（`tool_use`）与图片理解仍后置。**Gemini generateContent 多轮 + 流式适配已落地（2026-07-22）**：`GeminiGenerateContentTextAdapter`（`x-goog-api-key` 鉴权、model 在 URL 路径、流式切 `:streamGenerateContent?alt=sse`、`systemInstruction` + `contents` 角色映射 assistant→`model`、SSE `data:` 块为完整 JSON 响应、无 `[DONE]` 由字节 EOF 终止）；`makeRequest` 升为可动态派发的协议要求以承载 per-kind URL 方案（默认实现不变，其余 adapter 零改动）。结构化输出为尽力而为（仅 `generationConfig.responseMimeType`，adapter 不合成任何未登记外发 Prompt 文本；原生 `responseSchema` 严格模式后置）。至此文本闭集五 kind 全部可派发，`unsupportedProvider` 保留为未来 kind 的错误词汇表。图片 / TTS / embedding 仍后置。
...
- 2026-06-27：登记语伴 Style 受控片段注入 + i+1 下投影（LM03-S4a，语伴系列收尾）。原因：把 LM02-S2 surface Style 印记（句长/词汇丰富度/正式度，派生自母语写作）经 Ability band 注入语伴 system prompt。主要事实：(1) 新 included descriptor `AIRequestContentDescriptor.curatedLearnerStyle`，**复用 `.companionConversation` capability、不新增 capability**；`AIRequestProjections.companionConversation(..., hasStyleInjection:)` 注入时 includedContent 加 `.curatedLearnerStyle`。(2) **i+1 下投影 v1 mapping**：`CompanionStyleProjection`（LangoTraceLearnerModel 纯函数）把 surface 指标 + **只读** band 量化为 `CompanionStyleDescriptor`（Core）——正式度镜像语域、复杂度 ceiling=`band.estimatedLevel` 封顶，不把母语复杂度强加初学者（idea-01 §13.5）；`minimumSampleCount` 阈值不足返 nil。(3) 注入 Prompt = 复用 `builtin.companion.system.v1` 加性 `styleDescriptor` 受控片段（`<<<STYLE>>>` 引用非指令 + `.styleGroundedPersona` directive，进 `docs/prompts/companion/style.md`）。(4) **隐私：Style 块零原始用户内容**（枚举派生类别 + CEFR ceiling）→ **无需 PII scrub**；复用 S2b-1 两层 consent（`uses_learner_profile` 语义澄清为 Memory+Style 一闸）+ 一次性预览披露 Memory+Style；**无新 migration / 无新 capability / 无新外发类目语义升级**。(5) band 红线只读（仅 SELECT `dictionary_lookup_events`，不触 band 重估 / 解释模式 derive 写路径；源级 grep + 行为守卫）。影响范围：LangoTraceCore（`CompanionStyleDescriptor` + descriptor）/ LearnerModel（`CompanionStyleProjection`）/ AI（`CompanionPromptRegistry`·`CompanionConversationEngine`·`AIRequestProjections`）/ UI（`RequestPreviewCardModel.label` + 本地化）/ App Shell、本规范、architecture/002、page-inventory、idea-01 §13.5/§13.9、idea-03 §3.11、prompts/companion/style.md。是否需要 ADR：否，受 ADR-006 §6 / ADR-008 / 决策 #10 约束、沿用 ADR-005；详见 `docs/plans/done/2026-06-27-feature-lm03-s4a-companion-style-injection.md`。
- 2026-07-22：Gemini generateContent 文本适配落地（文本 / 尽力而为结构化 JSON / 多轮 + 流式探针放行；`x-goog-api-key` 鉴权、model 在路径；`makeRequest` 升协议要求；图片 / TTS / embedding 与严格 `responseSchema` 后置）。详见 `docs/plans/done/2026-07-22-feature-gemini-text-adapter.md`。
