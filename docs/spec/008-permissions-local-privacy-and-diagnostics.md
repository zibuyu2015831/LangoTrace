...
- 2026-06-25：补充语伴权限边界（LM03-S1）。原因：S1 为纯文本聊天，用户仅打字 + 接收回复，**不录音、不触发麦克风 / Speech 权限**；逐句 TTS 朗读复用既有播放边界（TTS Provider 配置后单句播放不弹窗、无系统权限）但**本片暂未接朗读**（加性后续）。语音输入为远期，落地时须重评估 Speech / 麦克风权限与云端转写外发边界（默认 on-device 本地优先），事实源 `docs/architecture/notes/2026-06-25-companion-voice-input-and-engine-boundary-notes.md`。S1 零系统自动注入；Memory 注入 + 结构化 PII scrubbing = LM03-S2 实现前必决项。影响范围：LangoTraceUI、本规范、架构备忘录。是否需要 ADR：否，受 ADR-008 约束。
- 2026-06-26：落地语伴 Memory 注入两层隐私控制 + 结构化 PII scrubbing（LM03-S2b-1，§2 决策 #10 首个系统自动注入实例）。原因：语伴注入用户系统级生活事实是系统自动外发，须按 §2 的「首次开启的一次性预览披露 + 全局关 + per-conversation 开关」授权 UX 落地。主要事实：(1) 全局三态 consent（`CompanionMemoryConsent`，`UserDefaults` 非敏感）`notDecided` 期间不注入、首次触发一次性预览（非每次弹窗 / 非发送前拦截）；per-conversation 开关 v32 `companion_threads.uses_learner_profile`。(2) 结构化 PII scrubbing v1 = 手机号（`1[3-9]\d{9}`）+ 身份证号（18 位末位 X/数字），对**注入片段 + 历史回放 + 用户输入**统一 outbound 脱敏（`PIIScrubber`，确定性正则、中性母语占位防注入）；**outbound-only**——不改写持久 `companion_messages` / `learner_memory_facts`（存原文发脱敏），scrub 前后内容**绝不入诊断日志 / DB**。(3) 剩余风险：国际号 / 护照 / 银行卡 / 邮箱 / 地址不覆盖、可能误杀练习数字串（「宁少杀勿多杀」），主闸仍是注入门 + 一次性预览。影响范围：LangoTraceCore（`PIIScrubber` / consent / gate）、LearnerModel、AI、Data、UI、App Shell、本规范、spec/005·007、ADR-006·008、architecture/002。是否需要 ADR：否，受 ADR-008 / 决策 #10 约束、沿用 ADR-005；详见 `docs/plans/done/2026-06-26-feature-lm03-s2b1-companion-memory-injection.md`。
- 2026-06-26：落地语伴方案B 主动找话题 + **修复 S2b-1/方案A 既有 scrub 遗漏**（LM03-S2b-2，§2 决策 #10 第二个系统自动注入实例）。原因：语伴授权后自动从用户记录里挑一条开启话题，属系统自动注入外发。主要事实：(1) 一次性全局话题授权 `CompanionTopicSourcingConsent`（三态 `UserDefaults` 非敏感、首次预览披露非每次弹窗、notDecided/disabled 不找话题）+ 复用 v32 `uses_learner_profile`（语义统一「使用我的内容」，关一处同关画像与找话题）；**仅 send 回合内触发，绝不在 load/进入会话阶段外发**（保 S1 冷启动零外发）。(2) **修复既有漏洞**：S2b-1 的 `CompanionConversationEngine` scrub seam **遗漏 `seedEntryBody`**——方案A「围绕这条记录对话」带入的记录正文此前**未脱敏外发**；本片把 `seedEntryBody`（方案A）+ `broughtInRecords`（方案B）一并纳入 outbound PII scrubbing（手机号/身份证），存原文发脱敏、不入日志/DB。(3) 找话题只读 `entries.body` 纯列、排软删、**仅当前 space**（决策 #10 不发其他空间）；**不用 FTS**（须 rebuild 易陈旧 + body 折叠 AI `learning_text`）。(4) 残余风险：单条记录 PII 集中（仅手机号/身份证脱敏，缓解 = top-1 + 一次性预览 + 可关）、同会话可能重复引用最近记录。影响范围：LangoTraceCore / LearnerModel / AI / Data / UI、App Shell、本规范、spec/005·007、ADR-006·008、architecture/002 §4.14。是否需要 ADR：否，受 ADR-008 §6 / 决策 #10 约束、沿用 ADR-005；详见 `docs/plans/active/2026-06-26-feature-lm03-s2b2-companion-active-topic-finding.md`。
