...
- `practice_sessions.exercise_type` 的稳定值为 `shadowing / dictation / backtranslation`；v18 migration 已扩展 CHECK 约束。当前 shadowing（录音）、dictation（文本 attempt）与 backtranslation（文本 attempt）会话均真实启用。
- `practice_recordings` 保存录音 attempt metadata；完成态只允许引用 ready recording。
- `practice_text_attempts`（v23 migration）保存文本作答 attempt（听写、回译），schema 一次性覆盖两种方式：`exercise_type` CHECK 为 `('dictation','backtranslation')`，`diff_difference_count` / `diff_summary_json` 仅听写写入、回译恒 NULL（回译参考不判对错），`listen_count` 为低权重信息。**该“恒 NULL”由 `recordTextAttempt` 的 repository 级不变量强制（E5-D3）**：`recordTextAttempt` 是该表唯一写入路径，对 `backtranslation` 强制 diff 列写 NULL、listen_count 写 0，不依赖调用方自觉；理想形态 schema CHECK（L3）deferred 到下一次自然触及该表的迁移（见架构备忘录 §2.3）。attempt 文本是用户练习证据、本地主数据，不参与同步 / 默认导出 / 备份 / 诊断 / 任何外发路径，也不参与 TTS cache 清理 / LRU。
...
- `GRDBPracticeRepository.completedSentenceIDs(materialID:exerciseType:)` 是句子列表“已练”状态和“从第 n 句继续”CTA 的只读查询；完成状态按当前 exercise type 独立计算，不跨练习方式复用。听写 / 回译“已练”由未软删文本 attempt 派生（并入同一查询），shadowing 仍由 ready recording 派生。
- **E5 Slice 1 回译已落地（纯本地）**：在 `PracticeBacktranslationSessionView` 会话、attempt 持久化和隐私边界完成后才注册 mode 可用性，未仅点亮 segment。回译复用 `practice_text_attempts`，diff 列恒 NULL（参考不判对错，由 §2 repository 不变量强制）。参考表达三态为题面（母语原意）→ 作答 → 对照参考；参考富字段经只读 seam `LearningContentRepository.sentenceAnalysis(materialID:sentenceIndex:)` 读取（懒加载于 reveal，缺失降级 snapshot 兜底），全程零网络、不发新 AI 请求。可选 AI 点评（Slice 2）双重门禁待 E6 落地 + 单独隐私授权，未实现。
...
- 2026-06-18：§2 / §6 同步 E5 Slice 1 回译落地事实。原因：E5 Slice 1 落地回译本地参考对照闭环——新增只读 seam `LearningContentRepository.sentenceAnalysis(materialID:sentenceIndex:)`（按 position 返回富句分析，reveal 时懒加载 + snapshot 兜底）、`PracticeBacktranslationSessionView` 三态会话、`PracticeActions.submitBacktranslationAttempt` / `fetchSentenceAnalysis`，App 注册第三段 `.backtranslation`；§2 把回译从“仅 schema 预留”转为真实启用并写入 `recordTextAttempt` 的 diff-NULL repository 级不变量（E5-D3，schema CHECK L3 deferred），§6 把回译从“未实现”转为已落地并记录参考来源（既有分析、不发新请求）与 Slice 2 双重门禁。影响范围：句子列表 mode 可用性、回译会话、attempt 隐私边界、Data 读取 seam、`recordTextAttempt` 不变量。是否需要 ADR：否，沿用本地优先、用户显式触发、回译不判对错、参考只来自既有分析决策。
...
- 2026-06-18：§6 同步 E5 Slice 2 回译可选 AI 点评落地事实。原因：E5 Slice 2 落地回译可选 AI 点评——新增 `PracticeBacktranslationReviewService` + Prompt `builtin.practice.backtranslation_review.v1`（用户内容分隔符包裹防注入、无判定字段输出契约）、Core `PracticeBacktranslationReviewInput/Result/FailureCategory/Failure`、`PracticeActions.reviewBacktranslation`、回译会话「请 AI 点评」显式触发态（idle→sending→reviewed/failed，footnote 提前披露），并经 E6 预留 capability `practiceBacktranslationReview` 接入预览投影与 `ai_request_logs`；App Shell 在显式触发时解析默认 endpoint + 语言空间目标语言/水平，仅发送母语原意/作答/参考句/语言 code 五字段，写非敏感日志。§6 把「回译新 AI 请求」从边界提醒转为已落地事实：运行期保留显式触发（核心决策 10），点评不判对错、不持久化点评正文。影响范围：LangoTraceAI/Core/UI、App Shell、Prompt Registry、AI Provider 隐私规范、E6 请求预览/日志。是否需要 ADR：否，沿用 ADR-005 与核心决策 10。
