...
- 2026-06-25：补充语伴主回复语言边界（LM03-S1）。原因：语伴主回复**始终用当前语言空间目标语**（ADR-008 §2.1），以 `CompanionPromptRegistry` typed directive `alwaysReplyTargetLanguage` 结构化保证 + 请求携带稳定 target language code（非界面语 / 展示名反推）；入口标题 / 设置开关 / 失败态 / 冷启动问候等 chrome 用界面语言（本地化 key），其中冷启动问候是**界面语 UI 引导**（"打个招呼开始练习"）而非伪造的目标语 companion 发言（S1 冷启动零外发）。影响范围：LangoTraceUI（`CompanionChatView` / `CompanionChatPresentation`）、LangoTraceAI、本地化、本规范。是否需要 ADR：否，受 ADR-008 约束。
