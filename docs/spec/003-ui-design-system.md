...
- 2026-06-27：明确三端设置行构成决议（呼应 §411 卡片使用边界）。原因：设置页此前在能力卡之上裸放「学习画像」「语伴」两行，读作孤儿；据 §411「卡片仅用于重复 item、非默认外壳」，正确做法不是把两行都套卡片，而是区分交互类。决议：(1) 同构的「能力」项（语言空间/外观/AI Provider/同步/语伴…）统一走 `CapabilityStatusRow`（`ForEach(settingsCapabilities)` 重复 item，符合 §411），「列表行 → 详情页配置」；(2) 语伴本属能力类（有 on/off 状态、可配置），从裸 toggle 升级为 `SettingsCapability`（详情页承载开关与未来配置），不再特例；(3) 「学习画像」是 LM02 只读查看入口（非能力、无状态、不可配置），**刻意保留为顶部唯一非能力导航行**（Apple-ID-row 惯例），不强行套卡片也不并入其它能力——单一非能力行不构成孤儿。影响范围：设置主列表行构成、`CapabilityStatusRow` 使用边界、三端设置 section 与后续新增设置项归类。是否需要 ADR：否，属 §411 既有约束的应用澄清；详见 `docs/plans/done/2026-06-27-feature-companion-settings-capability.md`。
- 2026-06-27：澄清「卡片底（视觉容器）」与「升级为能力（交互模型）」是两件正交的事——补齐前一日决议第 (3) 条的措辞。原因：语伴升级后，「学习画像」成为设置列表里唯一不带 `.langoPanel` 卡片底的行（每个 `CapabilityStatusRow` 在 `body` 内自带 `.langoPanel(padding: 16)`），视觉上仍读作孤儿；前一日「不强行套卡片」一句意在「不升级为能力 / 不塞进能力模型」，而非「禁止卡片底」。决议：(1) 给 `LearnerProfileSettingsRow.body` 补 `.langoPanel(padding: 16)`，使其与能力行同款卡片底——这是这份列表「每行一张卡」既有设计语言的一致性补齐，不是把卡片当 section 外壳，仍符合 §411；(2) 学习画像**仍不是** `SettingsCapability`、无状态、无尾值，保持顶部唯一非能力导航行的语义不变。措辞修订：前一日第 (3) 条「不强行套卡片」专指「不升级为能力 / 不并入其它能力」，不否定 per-row 卡片底的视觉统一。影响范围：`LearnerProfileSettingsRow` 视觉容器、三端设置列表卡片一致性。是否需要 ADR：否；详见 `docs/plans/done/2026-06-27-feature-learner-profile-row-card-chrome.md`。
