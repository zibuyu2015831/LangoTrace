...
iPhone 记录详情中的母语原文和目标语言学习文本应优先服务阅读和练习，不应表现成带字段标题的表单。详情页可直接展示原文正文和目标语言正文，通过卡片间距、文本权重、编辑图标和上下文建立语义关系；避免常驻显示“母语记录 / 目标语言”这类系统概念标题。目标语言学习文本区应默认呈现为只读内容卡，而不是大面积固定高度输入框。默认状态只显示正文和右上角悬浮编辑按钮，避免删除字段标题后留下独立空按钮行；保存 / 重新分析等条件性动作不能以禁用图标的形式常驻。用户点编辑后进入专用 sheet 完成目标语言文本编辑。Sheet 内使用大面积 `TextEditor`、顶部取消 / 保存操作和动态标题“修改{目标语言}表达”，例如“修改英文表达”，适配长文本、键盘、光标和内部滚动；详情页本身不承载编辑态输入框。这样用户在同一屏中能同时看到原始记录、目标语言和逐句练习入口，长文本编辑也不会挤压学习内容。
...
- 2026-05-23：补充 iPhone 记录详情空间利用规则。原因：人工查看发现目标语言编辑区固定高度过大，且“听 / 练”两个按钮与正文同排时会挤压目标句和讲解宽度，影响短句卡片的阅读质感和首屏信息密度；`LearningMaterialEditorView` 已调整为默认只读、点击编辑后进入 sheet 编辑，sheet 标题使用“修改{目标语言}表达”，详情页正文块不再常驻显示“母语记录 / 目标语言”字段标题，编辑按钮改为右上角悬浮，避免字段标题删除后残留空按钮行；`SentencePairView` 已调整为顶部操作区加全宽正文，逐句“听”按钮不再打开解释型 sheet。影响范围：iPhone 记录详情目标语言编辑区、逐句分析卡片、后续句子级操作按钮。是否需要 ADR：否，属于组件视觉和可用性约束。
