...
- `.github/workflows/ci.yml` 是 `scripts/verify.sh` 的远程镜像，跑同一套 package 测试、三端构建、lint 和 `scripts/check-docs.sh`。`scripts/verify.sh` 仍是验证内容的权威定义。
- **测试场所策略（设备感知）**：本机开发设备是 MacBook Air M4（被动散热，长时间编译易发烫/超时），因此**重测试一律放 GitHub Actions**——全量验证、三端 `xcodebuild` 构建、跨多包测试、长时间套件都在 macOS runner 上跑；**本机只做轻量动作**——单个改动包的 `swift test --package-path`、`swiftformat .` / `swiftlint --no-cache` 自查、文档检查，不在本机跑全量 `verify.sh` 或三端构建。详见 [002 Runbook §1.1](../development/002-ci-and-branch-workflow.md)。
- **仓库可见性约定**：public 仓库 Actions 免费无上限，private 仓库 macOS 额度紧（10× 倍率，约 200 分钟/月）。仓库平时可保持 private，**需要跑 CI 前临时设为 public**（`Settings → Danger Zone → Change visibility`），跑完可设回 private；AI 在触发 CI 前应主动提醒用户切到 public。
...
- **禁止新增「读取源码文件断言子串」的测试**（即 `String(contentsOf: someSourceFileURL(named: "…"))`）。此类测试把实现细节当作事实源，任何等价重构（提取方法、移动逻辑、改变字符串格式）都会误触，而真实行为错误却可能通过。存量此类测试在触碰对应生产文件时必须同步替换为行为测试（bootstrap + 调用 + 断言错误类型 / 状态），无法立即替换时在任务方案中记录迁移 backlog 并说明风险。
...
- 2026-06-13：补充设备感知的测试场所策略与仓库可见性约定。原因：本机为 MacBook Air（被动散热），明确重测试放 GitHub Actions、本机只做轻量单包测试与 lint/format 自查，并约定跑 CI 前临时把 private 仓库设为 public 以用免费 macOS runner。影响范围：所有涉及测试的开发、`docs/development/002`、`docs/development/environment.md`、`docs/README.md` §1.4。是否需要 ADR：否。
...
- 2026-06-17：禁止新增「读取源码文件断言子串」的测试，并要求存量此类测试在触碰对应生产文件时同步替换为行为测试。原因：E0b Phase 6 测试债清偿确认此模式（`String(contentsOf: langoTraceAppSourceFileURL(named: "…"))`）把实现细节当作事实源，真实行为错误可能通过而等价重构会误触；`AppEnvironmentBootstrapTests` 已有一例，E0b Phase 6 已将其替换为行为测试并登记其余存量文件为迁移 backlog。影响范围：`LangoTraceAppTests` 所有测试文件，以及后续在 `Packages/*/Tests` 中新增的测试。是否需要 ADR：否。
