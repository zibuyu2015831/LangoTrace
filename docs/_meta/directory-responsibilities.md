...
| `docs/_meta/documentation-constraints.md` | 轻量约束路由索引，帮助开发者快速定位应遵守的约束来源；不是新的产品决策源、架构事实源或裁决源 | 受保护规则 | 高频治理约束增减、源文档路径变更、约束生命周期状态变更时更新 | 不作为产品决策源、架构事实源、执行裁决源或领域规范源；不复制领域规则正文；不把外部参考或 AI 推断升级为 blocker |
