...
find docs -maxdepth 3 -type f | sort
rg "TO[D]O|TB[D]|待补[充]|稍后完[善]|以后再[写]|待[定]" docs --glob '!worklogs/TEMPLATE.md'
git diff -- docs/README.md docs/project-initialization.md docs/architecture/001-initial-module-boundaries.md docs/technical-framework-roadmap.md docs/development-environment.md docs/worklogs/2026-05-17-chore-docs-code-alignment.md
...
```bash
rg -n '处于 App Shell 后|首次启动和语言空间功能前|当前仓库尚未创建：|SwiftUI App 工程。|Swift Package 模块。|RootView\.swift|后续可使用的验证命令示例|具体命令需在工程创建后|只需要落地|不需要落地|PrivacyStatus`' docs --glob '!docs/worklogs/**'
