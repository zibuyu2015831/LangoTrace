...
| 练习会话 | 听示范、录音、回放录音、保存完成态 | 练习 Tab 句子列表、记录详情 `练` | Implemented / Practice Recording | `PracticeSessionView`、`PracticeControlBar.swift`、`PracticeSessionViewModel.swift`、`PracticeActions.swift`、`LearningContentStore` | 通过 App 注入 `PracticeActions` 创建 / 恢复单句 shadowing session，保存 `PracticeSentenceSnapshot`；单句页听示范复用逐句 TTS playback coordinator；用户显式点击后请求麦克风并录音，开始录音前停止当前示范播放；文件写入 App 管理媒体资产目录和 GRDB practice recording metadata；单句页可回放最近 ready recording，完成态引用 ready recording；录音默认本地、不同步、不默认导出、不自动发送 AI Provider | 第一阶段不做发音评分、ASR、后台录音或录音上传；真实设备麦克风、回放音频路由和三端布局仍需人工验收 |
