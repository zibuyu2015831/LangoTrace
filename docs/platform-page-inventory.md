...
| 练习会话 | 听示范、录音、回放录音、保存完成态，并在同一篇记录内切换上一句 / 下一句 | 练习 Tab 句子列表、记录详情 `练` | Implemented / Practice Recording | `PracticeSessionView`、`PracticePromptCard.swift`、`PracticeControlBar.swift`、`PracticeSessionViewModel.swift`、`PracticeActions.swift`、`LearningContentStore` | 通过 App 注入 `PracticeActions` 创建 / 恢复单句 shadowing session，保存 `PracticeSentenceSnapshot`；单句页只保留内容卡、操作卡、底部句间导航和真正异常状态，不再显示重复 header、不可点击阶段 pill、常驻指导文案、缺少录音提示卡或底部 step card；顶部内容卡采用目标句常驻、`查看释义` 与 `查看讲解` 优先同行、中文释义按需完整展示且展开后不重复标题、讲解按段落留出间距的结构，隐藏释义或讲解时不预留大块空白；prompt card disclosure state 只是 route-local UI state，不进入 `PracticeSession`、recording metadata、TTS cache 或 media artifact；底部句间导航在当前 route 内替换相邻 seed，左侧 `上一句`、中间 `第 n / m 句`、右侧 `下一句`，录音中和录音回放中禁用切换，示范播放中切换会先停止旧播放；单句页听示范复用逐句 TTS playback coordinator；用户显式点击后请求麦克风并录音，开始录音前停止当前示范播放；文件写入 App 管理媒体资产目录和 GRDB practice recording metadata；单句页可回放最近 ready recording，重复录音后回放使用最近一次 ready recording，完成态引用 ready recording；录音默认本地、不同步、不默认导出、不自动发送 AI Provider | 第一阶段不做发音评分、ASR、后台录音或录音上传；真实设备麦克风、回放音频路由和三端布局仍需人工验收 |
...
| `PracticeSentenceListView` / `PracticeSessionView` | iPhone / iPad / macOS | Implemented / Practice Recording | `PhoneMainSupportingViews.swift`、`PracticeRouting.swift`、`PracticePromptCard.swift`、`PracticeSessionViewModel.swift`、`PracticeControlBar.swift`、`PracticeActions.swift` | 记录练习句子列表、单句 route seed、句子快照、目标句优先的 prompt card、同行 disclosure controls、`查看释义` 按需显示完整中文释义、讲解按段落显示、听示范、跟读录音、ready recording 回放、完成态和三端共享 action seam |
