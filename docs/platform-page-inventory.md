...
| `PracticeSentenceListView` / `PracticeSessionView` | iPhone / iPad / macOS | Implemented / Practice Recording | `PhoneMainSupportingViews.swift`、`PracticeRouting.swift`、`PracticePromptCard.swift`、`PracticeSessionViewModel.swift`、`PracticeControlBar.swift`、`PracticeActions.swift` | 记录练习句子列表、单句 route seed、句子快照、目标句优先的 prompt card、短译文完整展示、长译文可展开、讲解按需查看、听示范、跟读录音、ready recording 回放、完成态和三端共享 action seam |
