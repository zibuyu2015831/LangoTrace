...
| 阅读 Tab | 管理当前语言空间阅读资料并进入阅读 | 底部 `阅读` Tab | Implemented / Reading CRUD / AI / TTS | `ReadingLibraryView`、`ReadingDocumentDetailView`、`ReadingLibraryStore`、`ReadingDocumentStore`、`ReadingMarkdownBlockRenderer`、`ReadingSelectionExplanationService` | iPhone 首屏改为资料库首页：标题、导入主动作、搜索、筛选、文档列表和删除恢复；空库时使用单一空状态卡承接 `粘贴文本` / `导入文件`。点开文档后进入独立阅读详情，详情内显示 Markdown / 纯文本阅读；右上角提供 `编辑` 入口，进入受控 editor sheet，阅读态不再在正文顶部重复显示 metadata block；保存后通过同一 Reading store / repository seam 更新标题与正文、刷新搜索索引与阅读 presentation、并失效旧 selection / explanation / TTS request。当前 compact 阅读详情默认以 `句子` 而不是整段作为学习操作对象：点击句子仅形成 sentence selection，不再在正文内联插入操作按钮；解释、听和未来扩展动作统一收进页面底部的非模态 learning panel，避免旧 bottom sheet 抢焦点。compact 面板当前显式区分 `hidden / collapsed / loading / content / failed`，新选中句子先进入 `collapsed`，不会直接展开旧 explanation 结果。解释请求按全文长度动态选择 `full_document / adjacent_paragraphs / current_paragraph` 上下文，并显式携带 sentence / paragraph fields；点击 `听` 后继续使用 `readingDocumentSentence` source 进入现有 TTS coordinator；已软删除文档需先恢复再编辑，若删除当前已选中文档则同步清空 reader 选中态；不自动发送全文、不自动 TTS、不做 EPUB/PDF/HTML | 后续需强化真实 TextKit / fragment selection engine、阅读位置、词典 UI、长文滚动人工验收和 VoiceOver 验收；不回退到 block 选中后弹阻断式 sheet 的旧交互 |
