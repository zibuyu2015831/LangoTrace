...
| 阅读 Tab | 管理当前语言空间阅读资料并进入阅读 | 底部 `阅读` Tab | Implemented / Reading AI / TTS | `ReadingLibraryView`、`ReadingLibraryStore`、`ReadingMarkdownBlockRenderer`、`ReadingDocumentStore`、`ReadingSelectionExplanationService` | 支持当前语言空间资料列表、搜索、粘贴导入、`.txt` / `.md` 文件导入、collection / tag 赋值和筛选、打开、软删除 / 恢复、Markdown / 纯文本 block 阅读、点击正文块形成显式 selection、用户点击 `解释` 后只发送 selection / sentence / limited context 给默认文本 Provider、点击 `听` 后使用 `readingDocumentSentence` source 进入现有 TTS coordinator；不自动发送全文、不自动 TTS、不做 EPUB/PDF/HTML | 后续需强化真实 TextKit / selection engine、阅读位置、词典 UI、长文滚动人工验收和 VoiceOver 验收 |
...
| 阅读资料库 route | 管理当前语言空间阅读资料并阅读 | 左侧 `阅读` | Implemented / Reading AI / TTS | `PadWorkspaceContentView`、`ReadingLibraryView`、`ReadingLibraryStore` | 复用同一 Reading action seam 和 GRDB repository，支持资料库列表、搜索、粘贴导入、`.txt` / `.md` 文件导入、collection / tag 赋值和筛选、打开、删除 / 恢复、Markdown / 纯文本阅读、显式选区解释和 reading sentence TTS；不自动发送全文或自动播放 | iPad 后续应强化 side inspector、真实选择引擎、阅读位置和 Dynamic Type / VoiceOver 验收 |
...
| Reading section | 管理阅读资料库并阅读文档 | Sidebar `Reading` 或 route `.reading` | Implemented / Reading AI / TTS | `MacWorkspaceContentView`、`ReadingLibraryView`、`ReadingLibraryStore`、`ReadingSelectionExplanationService`、`SentenceAudioPlaybackActions` | 复用当前语言空间 Reading repository，支持列表、搜索、粘贴导入、`.txt` / `.md` open panel 导入、collection / tag 赋值和筛选、打开、软删除 / 恢复、Markdown / 纯文本阅读、显式 selection explanation 和 `readingDocumentSentence` TTS；不做 EPUB/PDF/HTML、全文总结、全文朗读或后台播放 | 后续 Mac 应补 toolbar / command、键盘 selection、三栏 inspector 强化和长文性能人工验收 |
