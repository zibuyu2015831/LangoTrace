...
| 照片写作 | 从相册选取照片，剥离 EXIF GPS 元数据，生成缩略图，保存为照片写作记录并进入详情 | 记录 Hero 的 `用照片开始` | Implemented | `PhotoWritingView.swift`、`PhotoWritingActions.swift`、`PhotoImportPipeline`、`GRDBEntryPhotoAttachmentRepository` | 通过 `PhotosUI.PHPickerViewController` 从本地相册选取一张照片；App 在保存前剥离 EXIF GPS 元数据并生成缩略图；照片字节不发送 AI Provider、不上传、不同步；原始照片为用户主资产（`delete_after = NULL`），缩略图为可重建派生资产；保存完成后自动跳转到记录详情 | 不做 OCR、不做 AI 图片分析；多张照片附件、照片编辑和照片删除另开方案 |
...
| 逐句练习入口 | 从单句进入完整练习会话 | 记录详情每句的 `练` | Implemented / Practice Recording | `SentencePairView`、`PracticeRouting.swift`、`PracticeSessionView` | 入口携带 `PracticeSessionRouteSeed`，包含 entry、material、sentence identity、sentence index、target text hash、target language code、句子快照和同一篇记录内的轻量 sibling context；单句页只读取已有 LearningMaterial 翻译 / note，不新增 AI 语法分析请求 | `听` 只负责 TTS 示范，`练` 负责单句跟读录音完成；后续词典、单词本、评分、听写和回译仍需独立方案 |
| 练习 Tab | 从生活记录进入句子练习队列 | 底部 `练习` Tab | Implemented / Practice Recording | `PracticeView` in `PhoneMainSections.swift`、`PhonePracticeRows.swift`、`PracticeSentenceListView` | 首层改为可练习记录卡片列表，点击记录进入句子练习列表；不展示“从生活进入练习”标题，不混排听写 / 回译等未完成任务类型；没有 LearningMaterial 的记录不自动触发 AI 生成 | 记录卡片需要稳定高度、line limit 和尾部省略；不能把未来练习类型写成当前可用 |
| 练习会话 | 听示范、录音、回放录音、可重复重录，并在同一篇记录内切换上一句 / 下一句 | 练习 Tab 句子列表、记录详情 `练` | Implemented / Practice Recording | `PracticeSessionView`、`PracticePromptCard.swift`、`PracticeControlBar.swift`、`PracticeSessionViewModel.swift`、`PracticeActions.swift`、`LearningContentStore` | 通过 App 注入 `PracticeActions` 创建 / 恢复单句 shadowing session，保存 `PracticeSentenceSnapshot`；单句页只保留内容卡、操作卡、底部句间导航和真正异常状态，不再显示重复 header、不可点击阶段 pill、常驻指导文案、缺少录音提示卡或底部 step card；顶部内容卡采用目标句常驻、`查看释义` 与 `查看讲解` 优先同行、中文释义按需完整展示且展开后不重复标题、讲解按段落留出间距的结构，隐藏释义或讲解时不预留大块空白；prompt card disclosure state 只是 route-local UI state，不进入 `PracticeSession`、recording metadata、TTS cache 或 media artifact；底部句间导航在当前 route 内替换相邻 seed，左侧 `上一句`、中间 `第 n / m 句`、右侧 `下一句`，录音中和录音回放中禁用切换，示范播放中切换会先停止旧播放；单句页听示范复用逐句 TTS playback coordinator；用户显式点击后请求麦克风并录音，开始录音前停止当前示范播放；文件写入 App 管理媒体资产目录和 GRDB practice recording metadata；单句页可回放最近 ready recording，重复录音后回放使用最近一次 ready recording，已有录音时主按钮显示 `再录一次` 并继续允许录音；“已练过”由 ready recording 派生，不要求手动标记完成；录音默认本地、不同步、不默认导出、不自动发送 AI Provider | 第一阶段不做发音评分、ASR、后台录音或录音上传；真实设备麦克风、回放音频路由和三端布局仍需人工验收 |
...
| 阅读资料库 route | 管理当前语言空间阅读资料并阅读 | 左侧 `阅读` | Implemented / Reading CRUD / AI / TTS | `PadWorkspaceContentView`、`ReadingLibraryView`、`ReadingLibraryStore`、`ReadingDocumentStore` | iPad 阅读工作台现采用“资料库侧栏 + 中央阅读画布 + 次级学习 inspector”的承载：左栏聚焦导入、搜索、筛选和文档列表，中栏为主阅读区，右栏保留解释反馈和选中文本上下文；继续复用同一 Reading action seam 和 GRDB repository，支持资料库列表、搜索、粘贴导入、`.txt` / `.md` 文件导入、collection / tag 赋值和筛选、打开、标题 / 正文编辑、删除 / 恢复、Markdown / 纯文本阅读、句子级显式选择解释和 reading sentence TTS；iPad 与 iPhone 共享 sentence-first selection / dynamic context request seam，但结果继续由右侧 inspector 承载，不改成移动端底部面板；编辑通过受控 sheet 进入，不在阅读正文内联切换模式；删除当前已选中文档时会同步清空 reader 与 inspector 状态，恢复后不自动重新选中；不自动发送全文或自动播放 | 后续仍需强化真实选择引擎、阅读位置、Split View / Stage Manager 人工验收和 Dynamic Type / VoiceOver 验收；不回退到三列同权挤压主阅读区的验证态布局 |
...
| Reading section | 管理阅读资料库并阅读文档 | Sidebar `Reading` 或 route `.reading` | Implemented / Reading CRUD / AI / TTS | `MacWorkspaceContentView`、`ReadingLibraryView`、`ReadingLibraryStore`、`ReadingDocumentStore`、`ReadingSelectionExplanationService`、`SentenceAudioPlaybackActions` | macOS 阅读页现采用更明确的三栏工作台：左栏是资料库与导入管理，中栏是独立阅读画布，右栏是持续可见的学习 inspector；继续复用当前语言空间 Reading repository，支持列表、搜索、粘贴导入、`.txt` / `.md` open panel 导入、collection / tag 赋值和筛选、打开、标题 / 正文编辑、软删除 / 恢复、Markdown / 纯文本阅读、句子级显式 selection explanation 和 `readingDocumentSentence` TTS；解释请求与 iPhone / iPad 对齐到同一 sentence-first / dynamic-context seam，桌面端继续使用 persistent side inspector 而不是移动端底部面板；编辑通过受控 sheet 进入，不做正文页内联读写双模式；删除当前已选中文档时会同步清空 reader 与 inspector 状态，恢复后不自动重新选中；不做 EPUB/PDF/HTML、全文总结、全文朗读或后台播放 | 后续 Mac 仍应补 toolbar / command、键盘 fragment selection 和长文性能人工验收；不回退到视觉统一但职责不清的旧三栏结构 |
...
| `PracticeSentenceListView` / `PracticeSessionView` | iPhone / iPad / macOS | Implemented / Practice Recording | `PhoneMainSupportingViews.swift`、`PracticeRouting.swift`、`PracticePromptCard.swift`、`PracticeSessionViewModel.swift`、`PracticeControlBar.swift`、`PracticeActions.swift` | 记录练习句子列表、单句 route seed、句子快照、目标句优先的 prompt card、同行 disclosure controls、`查看释义` 按需显示完整中文释义、讲解按段落显示、听示范、跟读录音、ready recording 回放、可重复重录和三端共享 action seam |
