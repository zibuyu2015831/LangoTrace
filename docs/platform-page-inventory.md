...
| 照片写作 | 从相册选取照片，剥离 EXIF GPS 元数据，生成缩略图，保存为照片写作记录并进入详情 | 记录 Hero 的 `用照片开始` | Implemented | `PhotoWritingView.swift`、`PhotoWritingSaveCoordinator.swift`、`PhotoWritingActions.swift`、`PhotoImportPipeline`、`GRDBEntryPhotoAttachmentRepository` | 通过 `PhotosUI.PHPickerViewController` 从本地相册选取一张照片；App 在保存前剥离 EXIF GPS 元数据并生成缩略图；照片字节不发送 AI Provider、不上传、不同步；原始照片为用户主资产（`delete_after = NULL`），缩略图为可重建派生资产；只有 Entry 创建和照片附件导入都成功后才跳转到记录详情，导入失败会停留在照片写作页并回滚已创建 Entry | 不做 OCR、不做 AI 图片分析；多张照片附件、照片编辑和照片删除另开方案 |
...
| iPhone 设置列表 | 查看语言空间、界面语言、外观、AI、同步、本地数据、隐私、导入导出等能力边界 | 顶部 gear | Implemented | `SettingsView` in `PhoneMainSections.swift` | 设置项多数为能力边界；AI Provider 已接入本地配置保存和 Keychain；界面语言和外观可切换设备级 preference；导入导出只说明能力边界，不打开文件面板；设置主列表显式隐藏通用 `CapabilityStatusBadge`，不把开发阶段 `可用` / `本地优先` / `待配置` 角标作为用户信息展示 | 设置不能成为一级 Tab；文案要面向用户而非工程排期；后续真实设置健康状态必须通过独立 projection 设计，不能复用开发 completion marker |
...
| 设置详情 | 查看单项能力的当前状态、下一步和副作用边界 | 设置列表 row | Implemented / Unavailable | `SettingsCapabilityDetailView.swift` | AI Provider 已接入本地配置保存和 Keychain；同步仍是真实级 mock 配置入口；界面语言和外观可切换设备级 preference；不保存真实导入导出配置；设置列表 row 隐藏通用状态 badge，详情页和能力反馈组件仍可使用状态 badge 表达具体状态 | 后续真实 Provider 请求、同步、导入导出接入后逐项复查；未来设置列表当前值 / 健康状态需按 `docs/architecture/notes/2026-05-24-settings-status-projection-notes.md` 重新设计 |
...
| 右侧学习面板 | 随当前 route 展示上下文；记录 / 练习 route 展示当前句子、记忆提取、练习入口、空间设置、请求预览 | 右侧面板 | Implemented / Local Projection | `PadMainView.learningPanel`、`PadLearningPanelView` | `RequestPreviewCard` 只出现在 entry 学习上下文；设置、记忆、导入导出和语言空间 route 显示对应低干扰上下文，不发送数据；练习 route 可展示当前单句上下文但不直接持有 recorder 或文件路径 | 右侧内容必须匹配主区 route，不能在设置页继续展示当前记录学习内容；右侧面板容器需保留外侧 gutter，内容块需保留额外 trailing inset，避免贴近屏幕边缘 |
...
| `PracticeSentenceListView` / `PracticeSessionView` | iPhone / iPad / macOS | Implemented / Practice Recording / Mode Foundation | `PhoneMainSupportingViews.swift`、`PracticeRouting.swift`、`PracticeSentenceListPresentation.swift`、`PracticePromptCard.swift`、`PracticeSessionViewModel.swift`、`PracticeControlBar.swift`、`PracticeActions.swift` | 记录练习句子列表、mode availability、按 exercise type 计算的已练状态、续练 CTA、单句 route seed、句子快照、目标句优先的 prompt card、同行 disclosure controls、`查看释义` 按需显示完整中文释义、讲解按段落显示、听示范、跟读录音、ready recording 回放、可重复重录和三端共享 action seam |
