...
| 记录详情 | 阅读和编辑原始记录、生成学习材料、编辑 learning text、重新分析和逐句进入练习 | 最近记录、保存后、照片写作创建后 | Implemented | `EntryDetailView`、`EntryDetailHeader.swift`、`SentencePairView`、`LearningMaterialGenerationActions` | iPhone / iPad / macOS 均通过共享 `EntryDetailView` 接入真实 `生成学习材料` action；无 material 时只有一个核心 AI 动作，点击后经 App Shell 发送当前文本给已配置 AI Provider 并保存 GRDB LearningMaterial；原始 Entry 正文和 learning text 均使用动态文本卡片与 sheet 编辑；原文编辑只保存本地 Entry，不自动外发，已有 material 会显示“基于旧记录”并提供显式重新生成；learning text 编辑后可 `重新分析`；记录详情不再在句子列表后追加 practice candidate 信息卡，练习入口收敛到每句 `练` 和练习 Tab / 平台练习列表 | 不恢复本地预览主按钮；不恢复“进入练习”候选信息区；生成动作必须披露 AI Provider 边界；iPad / macOS 仍需后续人工验收大屏布局和创建入口保存失败恢复 |
