...
| 练习会话 | 听示范、录音、保存完成态 | 练习 Tab 句子列表、记录详情 `练` | Implemented / Practice Recording | `PracticeSessionView`、`PracticeControlBar.swift`、`PracticeSessionViewModel.swift`、`PracticeActions.swift` | 通过 App 注入 `PracticeActions` 创建 / 恢复单句 shadowing session，保存 `PracticeSentenceSnapshot`；用户显式点击后请求麦克风并录音，文件写入 App 管理媒体资产目录和 GRDB practice recording metadata，完成态引用 ready recording；录音默认本地、不同步、不默认导出、不自动发送 AI Provider | 第一阶段不做发音评分、ASR、后台录音、录音上传或真实录音回放控制面扩展；真实设备麦克风和三端布局仍需人工验收 |
...
| `PracticeSentenceListView` / `PracticeSessionView` | iPhone / iPad / macOS | Implemented / Practice Recording | `PhoneMainSupportingViews.swift`、`PracticeRouting.swift`、`PracticeSessionViewModel.swift`、`PracticeControlBar.swift`、`PracticeActions.swift` | 记录练习句子列表、单句 route seed、句子快照、跟读录音、ready recording 完成态和三端共享 action seam |
