...
| 练习会话 | 听示范、录音、回放录音、保存完成态，并在同一篇记录内切换上一句 / 下一句 | 练习 Tab 句子列表、记录详情 `练` | Implemented / Practice Recording | `PracticeSessionView`、`PracticeControlBar.swift`、`PracticeSessionViewModel.swift`、`PracticeActions.swift`、`LearningContentStore` | 通过 App 注入 `PracticeActions` 创建 / 恢复单句 shadowing session，保存 `PracticeSentenceSnapshot`；单句页只保留内容卡、操作卡、底部句间导航和真正异常状态，不再显示重复 header、不可点击阶段 pill、常驻指导文案、缺少录音提示卡或底部 step card；底部句间导航在当前 route 内替换相邻 seed，左侧 `上一句`、中间 `第 n / m 句`、右侧 `下一句`，录音中和录音回放中禁用切换，示范播放中切换会先停止旧播放；单句页听示范复用逐句 TTS playback coordinator；用户显式点击后请求麦克风并录音，开始录音前停止当前示范播放；文件写入 App 管理媒体资产目录和 GRDB practice recording metadata；单句页可回放最近 ready recording，重复录音后回放使用最近一次 ready recording，完成态引用 ready recording；录音默认本地、不同步、不默认导出、不自动发送 AI Provider | 第一阶段不做发音评分、ASR、后台录音或录音上传；真实设备麦克风、回放音频路由和三端布局仍需人工验收 |
...
| 练习详情 | 在主区进行单句跟读录音 | 句子 `练` 或学习面板练习入口 | Implemented / Practice Recording | `PracticeSentenceListView`、`PracticeSessionView`、`PracticeControlBar.swift`、`PracticeActions.swift` | iPad 共享同一 `PracticeSessionRouteSeed`、practice repository、recording service、示范 TTS playback coordinator 和媒体资产写入 / 回放路径；当前主流程为记录 -> 句子列表 -> 单句跟读 / 听示范 / 录音 / 回放 / 完成 | 大屏布局和 Stage Manager resize 需要人工验收；后续键盘快捷键和评分不得分叉业务状态 |
...
| Practice route | 执行单句跟读录音会话 | Practice item、句子列表或 entry detail | Implemented / Practice Recording | `PracticeSentenceListView`、`PracticeSessionView` reused in `MacWorkspaceContentView.practice` | macOS 复用同一 practice action seam、GRDB practice repository、示范 TTS playback coordinator、App 管理媒体资产、录音回放 resolver 和麦克风权限配置；录音默认本地，不自动上传、不默认导出 | 后续需要桌面 toolbar / command / shortcut 设计和真实设备麦克风验收；当前不做后台录音、ASR 或评分 |
...
| `PracticeSentenceListView` / `PracticeSessionView` | iPhone / iPad / macOS | Implemented / Practice Recording | `PhoneMainSupportingViews.swift`、`PracticeRouting.swift`、`PracticeSessionViewModel.swift`、`PracticeControlBar.swift`、`PracticeActions.swift` | 记录练习句子列表、单句 route seed、句子快照、听示范、跟读录音、ready recording 回放、完成态和三端共享 action seam |
