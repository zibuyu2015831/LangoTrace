...
| Practice section | 查看所有 entry 的练习入口 | Sidebar `Practice` | Implemented / Practice Recording | `MacWorkspaceContentView.practiceContent`、`PhonePracticeRows.swift` | 有 rendering 的记录显示可进入的练习卡片，点击后进入句子列表；不混排未实现的听写 / 回译任务 | 桌面密度和键盘操作仍需后续人工验收；不能把真实同步 / 导出录音写成已完成 |
| Practice route | 执行单句跟读录音会话 | Practice item、句子列表或 entry detail | Implemented / Practice Recording | `PracticeSentenceListView`、`PracticeSessionView`、`PracticePromptCard.swift` reused in `MacWorkspaceContentView.practice` | macOS 复用同一 practice action seam、GRDB practice repository、示范 TTS playback coordinator、prompt card disclosure UI state、App 管理媒体资产、录音回放 resolver 和麦克风权限配置；录音默认本地，不自动上传、不默认导出 | 后续需要桌面 toolbar / command / shortcut 设计和真实设备麦克风验收；当前不做后台录音、ASR 或评分 |
