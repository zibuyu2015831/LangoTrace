...
- 2026-05-20：用户在 iOS 模拟器复测真实保存成功后反馈，按钮内 `ProgressView` / “正在保存...”的瞬态变化过快，视觉上显得不稳定。后续 bug 方案 `2026-05-20-bug-ios-simulator-keychain-save-failure.md` 已将当前交互修订为：保存按钮保持“保存配置”与 `lock.shield` 稳定显示，保存中、成功、失败只通过底部状态面板表达，重复点击由代码 guard 防抖。本文后续阶段记录中关于“按钮内 `ProgressView`”的描述保留为当时实施历史，不再代表当前推荐交互。
