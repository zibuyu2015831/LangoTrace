...
- 2026-05-27：完成后严格复查发现一处遗漏：UI draft 仍以 text probe readiness 作为测试按钮和 snapshot 的硬前置，导致“文本 endpoint 尚未配置完整、embedding endpoint 使用独立凭证且配置完整”的场景无法触发 embedding probe，不满足第 12.4 条覆盖要求。已补 TDD 回归：`completeIndependentEmbeddingProbeIsAllowedWhenDraftTextCredentialIsMissing` 和 `configurationServiceRunsDraftEmbeddingProbeWhenTextEndpointIsAbsent`；实现上允许 draft text endpoint 为空，text / JSON row 返回 `.notConfigured`，embedding probe 继续执行且不持久化。已运行并通过：`swift test --package-path Packages/LangoTraceUI --filter completeIndependentEmbeddingProbeIsAllowedWhenDraftTextCredentialIsMissing`、`swift test --package-path Packages/LangoTraceAI --filter configurationServiceRunsDraftEmbeddingProbeWhenTextEndpointIsAbsent`、`swift test --package-path Packages/LangoTraceUI --filter AIProviderSettingsProbeTests`、`swift test --package-path Packages/LangoTraceAI --filter AIProviderConfigurationServiceTests`。
