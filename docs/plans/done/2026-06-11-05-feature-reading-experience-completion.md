...
类型：feature
创建日期：2026-06-11
最后更新日期：2026-06-17（Phase 5 文档收口完成，方案移入 done/）
...
2026-06-17：全部五个 Phase 实施完成（本机 MacBook Air 执行）。

- **Phase 1 (DATA-08)**：commit `1a59ccf`。`rebuildStructure` 改用 `ReadingTextSegmentation.segmentSentences` 真实分句；`importInlineDocument` 同事务内调用 `rebuildStructure`；v21 migration 为 `reading_structure_blocks` / `reading_sentences` 新增 `content_revision` 列；存量占位结构按 R1 Plan §5 早期重构原则不做兼容，由迁移后首次重建覆盖。7 条测试（`ReadingStructureBuildTests`）全绿。
- **Phase 2（进度 + 收藏）**：commit `837ef18`。v22 migration 为 `reading_documents` 新增 7 列（`is_favorite`、`reading_progress_percent`、`last_read_block_index`、`last_read_character_offset`、`last_read_structure_version`、`last_read_content_revision`、`read_completed_at`、`body_word_count`）；`GRDBReadingLibraryRepository` 实现 `setFavorite` / `updateReadingProgress` / `listDocuments(filter:)` + `summary(from:db:)` 新字段读取；`ReadingLibraryStore` 新增 `selectedLibraryFilter` 与收藏乐观更新；`ReadingViewComponents` 新增资料库行三段元信息 + 收藏星标 + `全部/收藏` chips；`ReadingWordCounter`（Core 纯函数）CJK / 非 CJK 双路字数计算；`AppEnvironment` 装配 `setFavorite` / `updateReadingProgress`。8 条 Data 测试 + 4 条 Core 测试 + 13 条 UI 测试全绿。
- **Phase 3（iPad inspector 折叠）**：commit `04a67d2`。`ReadingLayoutModel.canFoldInspector` 仅对 `.pad + showsPersistentInspector` 返回 true；`ReadingViews.swift` 新增 `@State isInspectorFolded`、折叠 / 展开按钮、折叠态底部 compact 面板 safeAreaInset；折叠 / 展开不触发 AI 或 TTS。5 条测试（`ReadingInspectorCollapseTests`）全绿。
- **Phase 4（解释语言模式面板控件）**：commit `d26780a`。`ReadingDocumentStore.currentExplanationMode` 变为 `@Published`；`switchExplanationMode(_:)` 在有选区时重发请求，切回旧 mode 命中缓存；`replaceDocument` 重置到 Level 派生默认；`ExplanationLanguageModePicker` 三档切换控件嵌入 `ReadingInspectorPane` 与 `ReadingCompactLearningPanel`（两处 `ReadingViews.swift` call site 均已接线）；本地化 key 四条（picker label + 三档名称，en + zh-Hans）。5 条测试（`ReadingExplanationModeSwitchTests`）全绿；475 条 UI package 测试全绿。
- **Phase 5（文档收口）**：commit 待提交。`docs/spec/012-reading-learning-domain.md` 新增 R1 变更记录；`docs/platform-page-inventory.md` 更新三个阅读页行与变更记录；`docs/architecture/notes/2026-06-06-reading-uitextview-per-block-patterns.md` §5 更新为"已采纳 → R1 Phase 3 落地"；本方案状态推进到 Implemented 并移入 done/。

**migration 专项审查说明**：v21 + v22 两次 migration 命中数据库 schema 变化触发条件。按 `docs/review/README.md` 判断：两项 migration 均为本地主数据或可重建派生数据扩展，不涉及同步 manifest、Keychain、AI Provider 隐私边界变化或核心 ADR 反转；App 仍处于早期无真实用户数据阶段，风险可接受。本次处置：在本方案实施记录中说明 migration 变化范围，不创建独立 review round；后续第一个涉及 migration 的合并前应人工复核 migration 单元测试在 CI 的通过状态。

**spike 处置说明**：`docs/reference/research/spikes/2026-06-06-bilingual-bridge-prompt-stability.py` 脚本已就绪但在当前开发机（MacBook Air，无真实 Provider 凭证的环境）无法执行。本次实施不设 spike 为代码门禁（控件不改变 Prompt 行为），按方案 §12 Phase 4 口径登记为人工验证 deferred 项：当用户有可控测试 Provider 和 API Key 时，在 macOS 真机环境运行该脚本，PASS / FAIL 与降级决策（FAIL 时 bridge 默认派生降级为 `sourceLanguage`）记录回 spikes 目录。
