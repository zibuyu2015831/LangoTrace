...
- 已批准按本方案进入 feature 纵向切片实施，并已按阶段完成检查、测试和 commit。
- 已批准本轮升级 iPhone 顶层导航为 `记录 / 阅读 / 练习 / 记忆`，并同步修订产品主参考、导航规范和页面清单。
- 本轮资料库 UI 已按方案交付列表、搜索、collection/tag filter、删除恢复入口；不做多选批量管理 UI，但 repository 支持 import batch 和 membership 幂等。
- 纵向切片后的词典导入与词状态、EPUB / PDF / HTML / Web clip adapter 均保留为后续独立 active plan 输入。

是否允许进入实现：是。当前状态已更新为 `Implemented`，本方案已按阶段完成检查、测试和 commit；最终完整验证与完成标准审计见第 17、18 节。
...
- 2026-06-01：阶段提交 `34fd925 Document reading vertical slice`：回写产品主参考、导航规范、页面清单、Data / AI / TTS spec、Reading domain spec、Prompt Registry 和 evidence 文档，并将 active plan 状态更新为 `Implementing`。阶段验证已运行 `find docs -maxdepth 3 -type f | sort`、`scripts/check-docs.sh`、`rg "TO[D]O|TB[D]|待补[充]|稍后完[善]|以后再[写]|待[定]" docs --glob '!plans/examples/*' --glob '!spec/examples/*'` 和 `git diff --check`。
- 2026-06-01：阶段提交 `c518283 Complete reading library management slice`：`ReadingLibraryStore` 和 `ReadingLibraryView` 新增 `.txt` / `.md` 文件导入、collection / tag 赋值和本地筛选；`ReadingViews` 增加 selection token 防止旧 AI / TTS completion 写回新 selection；`GRDBReadingLibraryRepository` 和 `AppEnvironment` 新增 reading AI explanation 非敏感 operation summary 写入。阶段验证已运行 `swift test --package-path Packages/LangoTraceUI --filter ReadingLibraryStoreTests`、`swift test --package-path Packages/LangoTraceData --filter GRDBReadingLibraryRepositoryTests`、`swift test --package-path Packages/LangoTraceUI --filter 'ReadingLibraryStoreTests|ReadingDocumentStoreAIAndTTSTests|ReadingPresentationTests'`、`swift test --package-path Packages/LangoTraceData --filter 'GRDBReadingLibraryRepositoryTests|AppDatabaseReadingMigrationTests'`、`xcodebuild -scheme LangoTrace-macOS -project LangoTrace.xcodeproj -destination 'platform=macOS' CODE_SIGNING_ALLOWED=NO build`、`scripts/check-docs.sh`、`rg "TO[D]O|TB[D]|待补[充]|稍后完[善]|以后再[写]|待[定]" docs --glob '!plans/examples/*' --glob '!spec/examples/*'` 和 `git diff --check`。
- 2026-06-01：阶段提交 `7fde1e3 Refactor reading UI components`：将 Reading library 面板、导入 sheet、metadata editor、inspector 和 block view 拆出，消除 Reading UI 新增代码的 SwiftLint serious，并保持 reading action seam 行为不变。阶段验证已运行 `swiftlint lint --strict Packages/LangoTraceUI/Sources/LangoTraceUI/ReadingViews.swift Packages/LangoTraceUI/Sources/LangoTraceUI/ReadingViewComponents.swift Packages/LangoTraceUI/Sources/LangoTraceUI/ReadingLibraryStore.swift Packages/LangoTraceUI/Sources/LangoTraceUI/ReadingActions.swift`、`swift test --package-path Packages/LangoTraceUI --filter 'ReadingLibraryStoreTests|ReadingDocumentStoreAIAndTTSTests|ReadingPresentationTests'`、`swift test --package-path Packages/LangoTraceData --filter 'GRDBReadingLibraryRepositoryTests|AppDatabaseReadingMigrationTests'`、`xcodebuild -scheme LangoTrace-macOS -project LangoTrace.xcodeproj -destination 'platform=macOS' CODE_SIGNING_ALLOWED=NO build` 和 `git diff --check`。
- 2026-06-01：阶段提交 `224af23 Fix AI provider formatting gate`：对既有 `AIProviderDraftConfiguration` 增加局部 lint 说明，解决完整验证中 SwiftLint 与 SwiftFormat 对同一构造器声明的格式门禁冲突。阶段验证已运行 `swiftlint lint --no-cache Packages/LangoTraceUI/Sources/LangoTraceUI/AIProviderDraftConfiguration.swift`、`swiftformat --lint Packages/LangoTraceUI/Sources/LangoTraceUI/AIProviderDraftConfiguration.swift --cache ignore`、`swift test --package-path Packages/LangoTraceUI --filter 'AIProviderSettingsTests|AIProviderSettingsProbeTests'` 和 `git diff --check`。
- 2026-06-01：阶段提交 `8ee033a Fix reading AI prompt lint`：拆分 reading selection explanation prompt 中过长字符串，保持 Prompt 语义和 JSON schema 不变。阶段验证已运行 `swift test --package-path Packages/LangoTraceAI --filter ReadingSelectionExplanationServiceTests`、`swiftlint lint --no-cache Packages/LangoTraceAI/Sources/LangoTraceAI/ReadingSelectionExplanationService.swift`、`swiftformat --lint Packages/LangoTraceAI/Sources/LangoTraceAI/ReadingSelectionExplanationService.swift --cache ignore` 和 `git diff --check`。
