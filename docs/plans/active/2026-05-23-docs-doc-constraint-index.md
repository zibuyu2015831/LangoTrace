...
3. 建立一份轻量文档约束索引，用于收录 LangoTrace 已经存在、需要高频遵守的文档治理约束，而不是创造新的产品或架构决策。
...
- 模板要求“证据与决策依据”和“验证命令”，但没有要求把核心约束写成可复查表格。
...
- 新增轻量文档约束索引，推荐落点为 `docs/_meta/documentation-constraints.md`，只收录已经存在于 `docs/README.md`、`docs/plans/README.md`、`docs/review/README.md`、`docs/_meta/documentation-system.md`、`docs/reference/README.md`、核心 spec 和 ADR 中的高价值规则。
- 新索引采用 Markdown 表格，不引入 JSON / YAML / 代码生成 pipeline。
- 每条约束至少包含：`id`、`kind`、`source`、`statement`、`scope`、`severity`、`enforced_by`、`verification_hint`。
- 首批索引优先收录文档治理、任务方案、review、reference 权威边界和验证入口等跨任务治理约束；隐私、存储、AI Provider 等领域规则只收录“应回到哪个权威文档检查”的触发型约束，不复制具体领域规则正文。
- 新索引明确它不是新的产品决策源；如果索引内容与原始 spec / ADR / product reference 冲突，以原始权威文档为准，并把冲突作为文档治理问题处理。
- 完成文档门禁验证，并记录验证结果。
...
- 新索引应只承载“已有权威规则的快捷索引”，不能成为凌驾于 ADR、spec、product reference 之上的新权威层。
...
- 新增 `docs/_meta/documentation-constraints.md` 可行，但它必须被定义为“约束索引 / 路由表”，不是新的领域事实源。
...
- 首批约束数量从“20 到 40 条”收紧为“12 到 24 条”，优先覆盖高频治理规则和跨任务触发器。
...
首批只收录高价值、已有明确来源的规则，建议控制在 12 到 24 条，不追求全量。第一版优先覆盖文档治理和跨任务触发器，不复制领域规范正文。初始分类建议：
...
```text
| ID | Kind | Source | Statement | Scope | Severity | Enforced by | Verification hint |
| --- | --- | --- | --- | --- | --- | --- | --- |
```
...
5. 运行文档验证命令。
6. 在本方案“实施记录”中记录实际修改、验证命令和结果。
7. 验证通过后，将本方案移入 `docs/plans/done/` 并更新状态。
...
rg -n "feature / bug / refactor / research / chore" docs/_meta/documentation-system.md docs/plans/README.md docs/plans/examples/task-plan-template.md
rg -n "docs/reference/_constraints_doc.html|constraints.json|audit_v2|variant-trigger|variant scatter" docs/_meta docs/plans docs/README.md docs/review/README.md
git diff --check
git status --short
```
...
- `docs/_meta/documentation-constraints.md`：新增约束索引。
...
- 2026-05-23：完成系统架构复查并写回方案：确认无 Swift 代码修改边界准确；收紧首批约束索引为 12 到 24 条治理和触发型约束；明确 `_meta` 索引不得复制领域规范正文；统一 `severity` 语义；补充文档体系变更验证命令。
...
- `docs/_meta/documentation-constraints.md` 已创建，且首批约束均有明确来源。
- 新增索引明确不替代 ADR、spec、product reference、review 和 plan 的权威关系。
- 已运行文档验证命令并记录结果。
- 本方案已移入 `docs/plans/done/`，状态为 `Verified` 或 `Done`。

## 17. 剩余风险

- Markdown 约束索引仍依赖人工维护，短期内不能保证自动发现所有文档漂移。
- 如果首批约束收录过多，后续维护成本会上升；本轮应优先收录高频、高风险、来源清晰的规则。
