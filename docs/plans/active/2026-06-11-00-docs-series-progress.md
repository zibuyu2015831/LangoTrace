...
第 3 批（语伴 = 完整聊天引擎；消费上方已退险的 Provider 多轮/流式 infra）
  ...
  LM03-S3 已拆 S3a/S3b（2026-06-26）
    ...
    S3b 已拆 S3b-1/S3b-2（2026-06-26，两半风险异质）
      🟢 S3b-1（对话记忆/滚动摘要：上下文窗口压缩 + 失效重建一致性，高风险）  ← 完整 plan + 双轮自审（2 P0+多 P1/P2 写回：失效 in-transaction / 触发纯函数 shouldSummarize / 隐私措辞订正 / 默认参数救调用 / fingerprint 去除）= Reviewed，待用户实现授权（v33 migration + 新自动 AI 外发<摘要>，隐私归语伴整体 opt-in 不新增 consent 门）
         ↓
      ⛔ S3b-2（对话小结 §3.12，中风险）  ← 边界登记（复用 S2a 提取 + S3b-1 摘要 infra + plan 10/11 deposit）
     ↓
