...
| LM03-S3b-2 | （未拆 active plan，边界登记于拆解文档） | 语伴对话小结（§3.12） | ⛔ **边界登记 / 门控未开**（2026-06-26 自 S3b 拆出）：本次对话小结（新词/表达 + 典型错误 + 记忆库候选），手动 + 可选会话结束触发。复用 S2a `CompanionExtractionEngine` 提取 + S3b-1 摘要 infra + plan 10/11 deposit 批量入口。进入实现前须拆 active plan + 双轮自审 + 授权 |
