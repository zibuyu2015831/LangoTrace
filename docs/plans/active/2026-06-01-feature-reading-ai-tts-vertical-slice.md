...
- 2026-06-01：当前阶段补齐实现缺口：`ReadingLibraryStore` 和 `ReadingLibraryView` 新增 `.txt` / `.md` 文件导入、collection / tag 赋值和本地筛选；`ReadingViews` 增加 selection token 防止旧 AI / TTS completion 写回新 selection；`GRDBReadingLibraryRepository` 和 `AppEnvironment` 新增 reading AI explanation 非敏感 operation summary 写入。阶段验证已运行 `swift test --package-path Packages/LangoTraceUI --filter ReadingLibraryStoreTests`、`swift test --package-path Packages/LangoTraceData --filter GRDBReadingLibraryRepositoryTests`、`swift test --package-path Packages/LangoTraceUI --filter 'ReadingLibraryStoreTests|ReadingDocumentStoreAIAndTTSTests|ReadingPresentationTests'`、`swift test --package-path Packages/LangoTraceData --filter 'GRDBReadingLibraryRepositoryTests|AppDatabaseReadingMigrationTests'`、`xcodebuild -scheme LangoTrace-macOS -project LangoTrace.xcodeproj -destination 'platform=macOS' CODE_SIGNING_ALLOWED=NO build` 和 `git diff --check`；提交前仍需运行文档检查。
