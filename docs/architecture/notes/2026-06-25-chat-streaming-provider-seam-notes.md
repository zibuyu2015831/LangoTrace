...
- `AIProviderAdapterKind` 五枚举 + `streamingChatBody` / `streamContentDelta` 协议方法（默认 `nil`）已是流式支持矩阵的 allowlist 维度：chat/responses 已实现；**anthropic 已由 LM03-S4b（`AnthropicMessagesTextAdapter`，2026-06-27）实现**；mimo 流式未验证暂继承 `nil`（= `unsupportedProvider`）；gemini 仍由工厂 `unsupportedProvider`。
- **鉴权策略已升为可动态派发的协议要求 `providerRequestHeaders(secret:)`（LM03-S4b）**：默认 OpenAI Bearer，Anthropic 覆盖 `x-api-key`+`anthropic-version`，mimo 覆盖 `api-key`。此前 mimo 的同名 `makeRequest` 重载因签名不匹配、存在体静态派发从不被调用（潜伏 bug），本接缝改造连带修复——新增非 Bearer 流式 Provider 只需覆盖该要求 + `streamingChatBody`/`streamContentDelta`，不必再加 `makeRequest` 重载。
...
- `ServerSentEventParser` 的字节级设计已天然支持 Anthropic SSE 事件流——**LM03-S4b 验证：Anthropic `data:` JSON 自带 `type` 字段足以判定 `content_block_delta`，命名 `event:` 行被 parser 丢弃不影响**，故只需新增 `AnthropicStreamDeltaExtractor` 分支与 `x-api-key` 鉴权，未重写行解析。Anthropic 无 `data: [DONE]` 终止符——流在字节 EOF 时由 `AIChatStreamingService` `continuation.finish()` 正常结束。

## 后续任务必须重新决策的问题

- **mimo 流式**：mimo 用 `api-key` 头（LM03-S4b 起经 `providerRequestHeaders` 真正生效），其 endpoint 是否支持 SSE 未验证；mimo 流式仍刻意 defer（继承 `streamingChatBody` 默认 `nil`）。后续验证 mimo SSE 后再决定是否放行。
...
- ~~不做 Anthropic / Gemini 适配（后置，走 `docs/workflows/add-ai-provider.md`）。~~ → **Anthropic 已由 LM03-S4b 落地（2026-06-27）**；Gemini 仍后置。
