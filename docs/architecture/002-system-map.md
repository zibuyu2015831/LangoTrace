...
- **未实现接缝**：逐句 TTS 朗读（暂缓，加性后续）；语音输入（远期，schema `input_modality`/`audio_artifact_id` 已预留，见 `docs/architecture/notes/2026-06-25-companion-voice-input-and-engine-boundary-notes.md`）；认知风格 Style v2（AI 校准、opt-in，后置）。**LM03 语伴系列主体全部落地**：对话记忆/滚动摘要 = S3b-1、对话小结 deposit = S3b-2；方案B 主动找话题 = S2b-2（§4.14）；Memory 注入 + PII scrubbing = S2b-1（§4.13）；文本流式 UX + 温和复述 = S3a（§4.15）；Anthropic 多轮+流式适配 = S4b（§4.9）；**Style 受控片段注入 + i+1 下投影 = LM03-S4a 已落地**——`CompanionStyleProjection`（LearnerModel 纯函数）把 surface Style 印记 + **只读** band 量化为 `CompanionStyleDescriptor`（Core，正式度镜像 + 复杂度 ceiling=band.estimatedLevel 封顶），经 `CompanionPromptRegistry` 渲染 `<<<STYLE>>>` 块 + `.styleGroundedPersona` directive；同 `uses_learner_profile` 门装配（Memory+Style 一闸），Style 块零原始内容→无需 scrub、无新 migration/capability/外发类目，band 红线只读。
