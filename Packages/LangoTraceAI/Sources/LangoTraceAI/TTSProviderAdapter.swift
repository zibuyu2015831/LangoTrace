...
public struct OpenRouterMultimodalAudioSpeechAdapter: TTSProviderAdapter {
    ...
    public func makeRequest(input: TTSProviderAdapterRequestInput) throws -> URLRequest {
        ...
        request.setValue("https://github.com/zibuyu/LangoTrace", forHTTPHeaderField: "HTTP-Referer")
...
private func decodeOpenAIStyleMultimodalAudioResponse(_ data: Data) throws -> Data {
    guard let string = String(data: data, encoding: .utf8) else {
        os_log("LangoTrace Debug: TTS decode failed – response body is not valid UTF-8 (%{public}d bytes)", type: .error, data.count)
    ...
    if hasSSEChunks {
        ...
        if combinedAudioData.isEmpty {
            os_log("LangoTrace Debug: TTS SSE decode – found SSE chunks but extracted 0 audio bytes. First 200 chars: %{public}@", type: .error, String(string.prefix(200)))
            throw AIProviderHTTPClientError.invalidHTTPResponse
        }
    ...
    guard let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
        ...
        os_log("LangoTrace Debug: TTS non-SSE decode failed. First 200 chars: %{public}@", type: .error, String(string.prefix(200)))
        throw AIProviderHTTPClientError.invalidHTTPResponse
    }
    return decoded
}
