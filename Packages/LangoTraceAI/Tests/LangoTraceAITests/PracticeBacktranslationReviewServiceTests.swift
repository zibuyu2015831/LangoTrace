...
struct PracticeBacktranslationReviewServiceTests {
    ...
    // The "unsupported (gemini) adapter maps to unsupportedProvider" test was
    // removed 2026-07-22 with the Gemini wiring: every closed-set kind now
    // dispatches, so the scenario no longer exists (mapping code stays as the
    // error vocabulary for future kinds; the factory's exhaustive switch is
