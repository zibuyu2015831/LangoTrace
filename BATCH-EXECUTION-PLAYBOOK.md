...
- **E12 漂移复核结论（隔离子代理，2026-06-18）**：无核心决策/ADR 反转（决策 9 凭证不外发 + 备忘录 §4 渲染零 Keychain/零网络 = 硬边界）。关键漂移（实现按此）：① `AIProviderProfileStatus` 实为 **6 例（含 `.draft`）**，投影映射须覆盖；② 凭证存在性快照在 **`ai_provider_credentials.secret_presence`**（`AIProviderSecretPresence`：present/missing/inaccessible/unknown），写点 `markCredentialState`/`insert(_:credential)`（**纯 Data 侧**，非 Data/AI 接缝）；profile 状态/校验在 `ai_provider_profiles.status`/`last_validation_status` + endpoint v11 列；`GRDBAIProviderConfigurationRepository` **不 import Keychain**（渲染路径安全），`loadDefaultProfile()`/`credential(from:)` 零 Keychain 可读。③ migration 实为 **v26**，无新列需求（复用现列，**无 migration**）。④ UIV-08 行号漂移：PadMainSections **90–91**、MacMainView **246–247**、PadLearningPanelView **89–101**；**MacWorkspaceContentView 无 AI/sync 行**（其 .unavailable 是真实能力事实，非进度泄漏，不改）。⑤ `AIProviderStatus`/`SyncProviderStatus` 在 **Core `PrivacyStatus.swift`**（非 footer 文件）；`AIProviderStatus` 仅 4 例（无 missingKey/partiallyAvailable）→ 投影需映射或扩 footer 枚举。⑥ **LangoTraceUI 不依赖 LangoTraceSync** → footer 不能直读 `syncService.isEnabled`，须 App/Core 桥接为 `SyncProviderStatus`。⑦ `CapabilityStatusRow`（`LearningContentComponents.swift`）**无 trailing 值槽**，须加 param；`settingsCapabilities` 是 `GRDBLearningContentRepositoryBridge` 静态数组（忽略 spaceID）。⑧ `LocalDataUsageService` 不存在；DB 路径 `LanguageSpaceDatabaseLocation`（+ -wal/-shm），MediaArtifacts root 在 **App 层** `SentenceAudioPlaybackAssembly.defaultMediaArtifactsRoot()` → usage service 须注入两路径。⑨ 渲染层禁用词含英文 **`unavailable`/`not connected`**（`ThreePlatformPresentationCopyTests` 扫 `settings.`/`capabilityStatus.` 前缀 + UI 源码）；`PremiumUIBehaviorTests` 禁 UI 源码任何汉字 → 行值字符串必须 en+zh-Hans 本地化 key、UI .swift 零中文。⑩ 同步行真实值 = `syncService.isEnabled`（现恒 false）→「未启用」，如实投影非硬编码（SyncStatusProjection 已随 E11 通道 defer，E12 不依赖它）。设置渲染：iPhone `PhoneMainSections`，iPad/mac 共享 `LangoTraceSettingsSceneView`；界面语言/外观偏好已由 `UserDefaultsInterfaceLanguageStore`/`UserDefaultsAppearancePreferenceStore` 持久化（App 层）。
